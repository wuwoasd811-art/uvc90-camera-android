...
以后的 v4.3 可能会在另一台电脑上制作，也可能使用与 v4.2 不同的签名密钥。这种做法可以正常发布，但 Android 不允许把不同签名的 APK 直接覆盖安装到 v4.2 上。三星手机通常会提示“应用未安装”“签名冲突”或“更新与现有应用不兼容”。

如果 v4.3 使用了不同的签名密钥，请按以下步骤安装：

1. 先备份重要录像和设置。
2. 在手机上卸载 UVC90 Camera v4.2。
3. 下载并重新安装 v4.3 APK，把它作为一次全新安装。

卸载可能清除应用设置及应用私有数据。公共 `Movies/UVC90` 文件夹中的录像通常会保留，但仍建议提前单独备份。如果 v4.3 恰好沿用了 v4.2 的同一签名，则可以直接覆盖升级，不需要先卸载 v4.2。
