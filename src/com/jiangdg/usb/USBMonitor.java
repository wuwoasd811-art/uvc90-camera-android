...
public final class USBMonitor {
	...
	public synchronized void register() throws IllegalStateException {
        ...
        if (Build.VERSION.SDK_INT >= 34) {
          // UsbManager fills the device and grant result into this PendingIntent.
          // FLAG_IMMUTABLE prevents those extras on Android 14+ (notably Samsung),
          // making an accepted grant look like a cancellation.
...
//		if (DEBUG) XLogWrapper.v(TAG, "requestPermission:device=" + device);
	...
	/** Connect after permission was obtained by the host app's own receiver. */
	public synchronized boolean connectGrantedDevice(final UsbDevice device) {
		if (destroyed || device == null || !mUsbManager.hasPermission(device)) return false;
		processConnect(device);
		return true;
	}
